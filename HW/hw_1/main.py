...
class TestParse(unittest.TestCase):
    ...
    def testCookies9(self):
        const = parse_cookie('name=Dima;ориентация=гетеросексуал')
        self.assertEqual(const, {'name': 'Dima', 'ориентация': 'гетеросексуал'})

    def testCookies10(self):
        const = parse_cookie('name=Dima=User=God;')
        self.assertEqual(const, {'name': 'Dima=User=God'})
